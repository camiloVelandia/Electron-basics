    ...
    if (this.value.length > 0) {
      ...
      for (
        ...
        if (fileName.match(regex)) {
          thumbs[i].parentNode.classList.remove(
            "hidden"
          );
        } else {
...
function loadImages(images) {
  const imagesList = document.querySelector(
    "ul.list-group"
  );

  for (
    let i = 0, length1 = images.length;
    i < length1;
    i++
  ) {
...
function clearImages() {
  ...
  for (
    let i = 0, length1 = oldImages.length;
    i < length1;
    i++
  ) {
